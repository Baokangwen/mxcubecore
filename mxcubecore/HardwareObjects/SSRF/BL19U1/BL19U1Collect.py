...
class BL19U1Collect(AbstractCollect, HardwareObject):
    ...
    def _update_task_progress(self):
        ...
        while step_count < num_steps:
            if self._collecting is not True:
                logging.getLogger("HWR").debug("[COLLECT] Progress thread aborted because collection stopped.")
                break
    ...
    def emit_collection_finished(self):
        ...
        # ==========================================================
        # 🌟 步骤 1：先处理缩略图（利用它自带的 wait_for_file 锁死等待硬盘同步）
        # ==========================================================
        ...
        if num_images > 1:
            ...
            try:
                ...
                # ！！！这里非常关键：它会一直阻塞等待，直到最后一张图彻底同步到 NFS 硬盘上！！！
        ...
        try:
            ...
            if exp_type == "Mesh":
                ...
                logging.getLogger("HWR").info(f"[OFFLINE] 所有 {num_images} 张图片均已确认落盘，开始安全触发 XDS...")
                logging.getLogger("HWR").info(f"[OFFLINE] 执行命令: {cmd}")
                subprocess.Popen(cmd, shell=True, close_fds=True)
